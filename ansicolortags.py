...
* printc: like __builtin__.print, but with interpreting tags to put colors,
...
In this last example, an ``<el>`` tag is used to erase the current content of the line, useful to make a *dynamical* print ::
...
* To produce a [GNU Bash color aliases file](https://bitbucket.org/lbesson/bin/src/master/.color.sh) :code:`$ ANSIColors.py --generate --file ~/.color_aliases.sh`.
...
This script can normally detect if ANSI codes are supported :
  ...
  3. ``$ ANSIColors.py --help --ANSI`` : will force the use of colors, even if they seems to be not supported.
...
def _parser_default(description=_default_description, \
    ...
    Example:
     >>> parser = _parser_default(description='<DELETE>A description.',\
        epilog='The description will no begin by the tag DELETE, thanks to sprint preprocessing.',\
...
.. warning::
   ...
   The comportement of the function in case of false tags **is not perfect**.
...
def printc(chainWithBalises, left='<', right='>'):
    ...
    A shortcut to print sprint(chainWithBalises) : analyse all tags, and print the result."""
...
and when the computation is done, erase the current line (with <el> tag),
...
======<reset>
  ...
  * here on <neg>PyPi<Neg> : <u>https://pypi.python.org/pypi/ANSIColors-tags<U>,
  * and his documentation can be found here on <neg>Python Hosted<Neg> : <u>http://pythonhosted.org/ANSIColors-tags/<U>.
